...
export const getDetailedCoinData = async (coinId) => {
  try {
    ...
    const response = await axios.get(`https://api.coingecko.com/api/v3/coins/${coinId}?localization=false&tickers=true&market_data=true&community_data=false&developer_data=false&sparkline=false&vs_currency=usd`)
...
export const getCandleChartData = async (coinId, days ) => {
  try {
    const response = await axios.get(`https://api.coingecko.com/api/v3/coins/${coinId}/ohlc?vs_currency=usd&days=${1}`)
    return response.data;
  } catch (e) {
    console.log('6');
  }
